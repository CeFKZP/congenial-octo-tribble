...
Section('local_loss', 'local loss parameters').params(
    ...
    denoise_scaler=Param(float, 'denoising loss scaler', default=0.0),
)
...
class ImageNetTrainer:
    ...
    def create_model_and_scaler(self, arch, num_classes, gradient_depth, num_splits, dropout_p, no_detach,
        ...
        if distributed:
            model = ch.nn.parallel.DistributedDataParallel(
    ...
    def train_loop(self, epoch, log_level):
        ...
        for ix, (images, target) in enumerate(iterator):
            ...
            with autocast():
                ...
                bwd_activations = [net(target_onehot)
                                   for net in self.model.bwd_net]
    ...
    def _train_forward_net(self, forward_output, fwd_activations, bwd_activations, denoise_activations, target):
        forward_loss = self.loss(forward_output, target)
        loss_corr, loss_sim, loss_pred, loss_denoise = (ch.zeros((self.model.num_blocks - 1,), device=self.gpu) for _
                                                        in range(4))

        for i, acts in enumerate(fwd_activations):
            ...
            if self.local_pred_loss > 0:
                ...
                loss_pred[i] = self.loss(act_pred, target)
            if self.denoise_loss > 0:
                loss_denoise[i] = F.mse_loss(
                    denoise_activations[i], acts.detach())
    ...
    def _train_backward_net(self, fwd_activations, target):
        for proto_loss_step in range(self.greedy_train_steps):
            ...
            loss_proto = ch.zeros(
                (self.model.num_blocks - 1,), device=self.gpu)
            for i, acts in enumerate(fwd_activations):
                x_proto, labels = module_loss.get_proto_vectors(
                    acts.detach(), target, num_classes=self.model.num_classes)
                prototype = self.model.bwd_net[i](
                    ch.eye(x_proto.shape[0]).to(self.gpu))
                loss_proto[i] = 0.5 * \
    ...
    def launch_from_args(cls, distributed, world_size):
        if distributed:
            ch.multiprocessing.spawn(
                cls._exec_wrapper, nprocs=world_size, join=True)
        else:
            cls.exec(0)
